 ...
 *
 * Most of the code is taken from https://github.com/webpack-contrib/webpack-bundle-analyzer
 *
 * Modified for the Titanium webpack target
 */
...
export function analyzeBundle(stats, assetSources, outputPath) {
	// Sometimes all the information is located in `children` array
	if (isEmpty(stats.assets) && !isEmpty(stats.children)) {
	...
	for (const asset of jsAssets) {
		...
		if (!bundleInfo) {
			console.warn(
				`\nCouldn't parse bundle asset "${path.join(outputPath, asset.name)}".\n`
